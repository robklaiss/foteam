 ...
 * Functions File
 * 
 * Contains all the application's helper functions
 * 
 * @version 2.1.0
 */

// Make sure bootstrap is loaded before any other code
...
function ensure_session_started() {
    ...
    // If not active, try to start it only if headers not sent
    if (!headers_sent()) {
        // Include bootstrap to ensure session is properly configured
        if (!defined('BOOTSTRAP_LOADED') && file_exists(__DIR__ . '/bootstrap.php')) {
            require_once __DIR__ . '/bootstrap.php';
        }
        
        // If still not active, try to start it
...
function login_user($email, $password) {
    try {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            // For production server compatibility
            $_SESSION['authenticated'] = true;
            
            // Force session data to be written
            session_write_close();
            
            // Re-open session with the same ID
            session_id($current_session_id);
            
            // Verify session data was persisted
            if (!isset($_SESSION['user_id']) || $_SESSION['user_id'] != $user['id']) {
                // Session data wasn't saved correctly, try alternative approach
                if ($debug) {
                    error_log("Session data not persisted after write_close. Using alternative method.");
                }
                
                // Set session variables again
                $_SESSION['user_id'] = $user['id'];
                $_SESSION['username'] = $user['username'];
                $_SESSION['is_admin'] = (int)$user['is_admin'];
                $_SESSION['login_time'] = time();
        ...
        return ['success' => false, 'message' => 'Error del sistema al iniciar sesión. Por favor intente de nuevo.'];
    }
}

function is_logged_in() {
    // More robust check for logged-in status
...
function create_order($user_id, $customer_name, $customer_email, $customer_phone) {
    ...
    try {
        ...
        $order_stmt = $db->prepare("
            INSERT INTO orders (user_id, total_amount, name, email, phone)
            VALUES (:user_id, :total_amount, :name, :email, :phone)
        ");
        $order_stmt->bindValue(':user_id', $user_id, SQLITE3_INTEGER);
        $order_stmt->bindValue(':total_amount', $total_amount, SQLITE3_FLOAT);
        $order_stmt->bindValue(':name', $customer_name, SQLITE3_TEXT);
        $order_stmt->bindValue(':email', $customer_email, SQLITE3_TEXT);
        $order_stmt->bindValue(':phone', $customer_phone, SQLITE3_TEXT);
        $order_stmt->execute();
...
/**
 * Format a number as money in PYG (Paraguayan Guaraní) format
 * 
 * @param float $amount The amount to format
 * @param string $currency The currency symbol to use (default: '₲' for Guaraní)
 * @param int $decimals Number of decimal places (default: 0 for PYG)
 * @return string Formatted money string
 */
function format_money($amount, $currency = '₲', $decimals = 0) {
...
/**
 * Verify that a CSRF token matches the one stored in the session
 *
